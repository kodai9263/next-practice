...
interface Props {
  mode: "new" | "edit"
  title: string
  setTitle: (title: string) => void // 更新なのでvoidが必要
  content: string
...
export const PostForm: React.FC<Props> = ({
  ...
  const handleImageChange = async (
    ...
    const { data, error } = await supabase.storage
      ...
      // data.pathに、画像固有のkeyが入っているので、thumbnailImageKeyを格納する
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label 
          htmlFor="title" 
          className="block text-sm font-medium text-gray-700">タイトル
        </label>
        <input
          type="text"
          id="title"
      ...
      <div>
        <label
          htmlFor="content" 
          className="block text-sm font-medium text-gray-700">内容
        </label>
        <textarea
          id="content"
      ...
      <div>
        <label
          ...
          className="block text-sm font-medium text-gray-700">サムネイルURL
        </label>
        <input
          type="file"
      ...
      <div>
        <label 
          htmlFor="categories"
          className="block text-sm font-medium text-gray-700">カテゴリー
        </label>
        <CategoriesSelect selectedCategories={categories} setSelectedCategories={setCategories} disabled={isSubmitting}/>
      </div>
      <button 
        type="submit"
        disabled={isSubmitting}
